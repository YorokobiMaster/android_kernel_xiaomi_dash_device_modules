...
{
	WRITE_ONCE(ctx.ffa_dev, dev);
}

void mitee_smc_notify_deinit(void)
{
	WRITE_ONCE(ctx.ffa_dev, NULL);
}
			...
			uint16_t data_size,uint64_t data0, uint64_t data1, uint64_t data2)
{
	int ret;
	/* ffa ops */
	struct ffa_device *ffa_dev = READ_ONCE(ctx.ffa_dev);
	...
	if (!ffa_dev || !ffa_dev->ops || !ffa_dev->ops->msg_ops)
		return -ENODEV;
				...
				const void *data, uint16_t data_size)
{
	...
	while (left_size > 0) {
		...
		left_size -= curr_size;
		memcpy(data_buf, bytes + data_size - left_size - curr_size,
		       curr_size);
