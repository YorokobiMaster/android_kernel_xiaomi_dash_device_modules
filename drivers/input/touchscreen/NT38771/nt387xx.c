...
{
	nvt_get_fw_info();
	/* P16 dash: vendor string from lcd_id strap (0=TM, !=0=CSOT) */
	...
	/* hqsysfs is not part of the dash kernel ABI; keep the version in the driver log. */
}
/* P16 code for HQFEAT-89651 by liaoxianguo at 2025/3/24 end */
/*P16 code for HQFEAT-94426 by liuyupei at 2025/5/6 start*/
...
/* P16 dash: read lcd_id strap GPIOs to select CSOT/TM panel (mirrors stock p10u).
 * lcd_id_value1 == 0 -> TM, lcd_id_value1 != 0 -> CSOT.
 * Sentinel -1 marks an unreadable strap so tp_compare_ic() can fail the probe
 * instead of silently misclassifying the panel. */
static void nvt_read_lcd_id(struct device *dev)
{
	struct device_node *np = dev->of_node;
	...
	ts->lcd_id_gpio1 = of_get_named_gpio(np, "novatek,lcd_id-gpio1", 0);
	desc = gpio_to_desc(ts->lcd_id_gpio1);
	if (IS_ERR(desc)) {
		NVT_ERR("invalid lcd_id-gpio1 (%d)\n", ts->lcd_id_gpio1);
	} else {
		gpiod_direction_input(desc);
		ts->lcd_id_value1 = gpiod_get_raw_value(desc);
	}

	ts->lcd_id_gpio2 = of_get_named_gpio(np, "novatek,lcd_id-gpio2", 0);
	desc = gpio_to_desc(ts->lcd_id_gpio2);
	if (IS_ERR(desc)) {
		NVT_ERR("invalid lcd_id-gpio2 (%d)\n", ts->lcd_id_gpio2);
	} else {
		gpiod_direction_input(desc);
		ts->lcd_id_value2 = gpiod_get_raw_value(desc);
	}

	NVT_LOG("lcd_id_value1=%d, lcd_id_value2=%d\n", ts->lcd_id_value1, ts->lcd_id_value2);
}
...
{
	dsi_panel_gesture_enable(!!gesture_command);
}
...
int tp_compare_ic(void)
{
	...
	/* Primary: DSI panel lockdown bytes [0..1] ("F6" -> TM, "SB" -> CSOT) */
	if (ts->lockdown[0] == 'F' && ts->lockdown[1] == '6') {
		tm = true;
	} else if (ts->lockdown[0] == 'S' && ts->lockdown[1] == 'B') {
		tm = false;
	} else if (ts->lcd_id_value1 >= 0) {
		/* Fallback: lcd_id strap GPIO1 (0 -> TM, !=0 -> CSOT) */
		tm = (ts->lcd_id_value1 == 0);
	} else {
		NVT_ERR("no valid lockdown code and no valid lcd_id strap: cannot select panel\n");
		return -ENODEV;
	}

	NVT_LOG("%s lockdown=%c%c lcd_id_value1=%d -> %s\n", __func__,
		ts->lockdown[0], ts->lockdown[1], ts->lcd_id_value1, tm ? "TM" : "CSOT");

	if (tm) {
		BOOT_UPDATE_FIRMWARE_NAME = "novatek_nt38771_p10u_fw_tm.bin";
		MP_UPDATE_FIRMWARE_NAME = "novatek_nt38771_p10u_mp_tm.bin";
		NVT_LOG("TP is Tianma");
	} else {
		BOOT_UPDATE_FIRMWARE_NAME = "novatek_nt38771_p10u_fw_csot.bin";
		MP_UPDATE_FIRMWARE_NAME = "novatek_nt38771_p10u_mp_csot.bin";
		NVT_LOG("TP is CSOT");
	}
	return 0;
}
...
/*P16 code for HQFEAT-94426 by liuyupei at 2025/5/6 end*/
	/* P16 dash: panel select via lcd_id strap GPIOs instead of chosen/lcm_panel_id */
	nvt_read_lcd_id(&client->dev);
	/*P16 code for HQFEAT-102768 by p-zhaobeidou3 at 2025/4/16 start*/
	ret = tp_compare_ic();
	if (ret < 0) {
